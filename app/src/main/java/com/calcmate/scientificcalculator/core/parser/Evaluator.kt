...
class Evaluator(private val angleUnit: AngleUnit = AngleUnit.RADIAN) {
    ...
    fun evaluate(node: ASTNode): CalcResult = when (node) {
        ...
        is ASTNode.RandomNode -> CalcResult.RealResult(com.calcmate.scientificcalculator.core.math.RandomUtil.randomNumber())
    }
    ...
    private fun evaluateVariable(node: ASTNode.VariableNode): CalcResult {
        return com.calcmate.scientificcalculator.core.model.MemoryManager.recallVariable(node.name)
    }

    private fun evaluateAns(): CalcResult {
        return com.calcmate.scientificcalculator.core.model.MemoryManager.ans
    }
